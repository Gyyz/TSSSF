...
class Vocab(Configurable):
  ...
  def save_vocab_file(self):
    """"""
    vocab_file = os.path.join(self.save_dir,  os.path.basename(self.vocab_file))
  ...
  #=============================================================
  def save_vocab_file_stack(self):
    """"""
    vocab_file = os.path.join(self.save_dir, 'stack_' + os.path.basename(self.vocab_file))
    wc_list = self._counts_stack.most_common()
    vocab_file = self.vocab_file
    wc_list = self._counts.most_common()
  ...
  def _finalize(self):
    ...
    with tf.device('/cpu:0'):
      with tf.variable_scope(self.name):
        self.trainable_embeddings = tanh_const * tf.get_variable('Trainable_base', shape=(len(self._str2idx), self.embed_size), initializer=initializer)
        if self.stack:
          self.trainable_embeddings_stack = tanh_const * tf.get_variable('Trainable_stack', shape=(len(self._str2idx_stack), self.stack_embed_size), initializer=initializer)
  ...
  def embedding_lookup(self, inputs, pret_inputs=None, pret_inputs_stack=None, keep_prob=1, moving_params=None, top=False):
    ...
    if moving_params is not None:
      ...
      if self.stack:
        ...
        print('trainable embedding size\t', trainable_embeddings_stack.get_shape().as_list())
      keep_prob = 1
    else:
